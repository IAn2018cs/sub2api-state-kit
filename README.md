...
| 阶段 | 版本 / 形式 | 更新内容 |
| --- | --- | --- |
| 第一步 | 增量版（v0.1.x） | 先提供基于 Sub2API 的增量修改，方便已有用户合并账号级 STATE、Pro / Team 选择和动态守护 |
| 第二步 | 完整项目版（v0.2.0） | 提供完整源码和空白部署包，包含 Sub2API 与 STATE 功能，方便直接部署 |
| 第三步 | 插件版（v0.3.0） | 将功能放进 `.s2plugin`，在插件页面统一配置，降低安装、升级和日常操作成本 |
| 持续优化 | 插件版（v0.3.1 起） | 已加入双层代理基础能力、出口 IP 检测与运行日志；继续完善代理选择、账号识别和测试体验 |

**后续主要更新插件版。** 插件安装、升级和操作更方便，新功能优先在插件中完善；增量版和完整项目版保留现有入口，是否同步新功能以各版本说明为准。

## 插件优化 TODO

- [x] 提供增量版、完整项目版和插件版三种使用方式。
- [x] 插件支持账号级开关、Pro / Team、STATE 采集、固定业务出口复验、续期与异常守护。
- [x] 加入双层代理基础链路和运行日志：**部署机器 → 前置代理 → 动态 IP 代理 → 上游**。
- [ ] **完善双层代理的配置与排障体验。** 面向国内部署、无法直接连接部分动态 IP 服务的场景，方便先通过前置代理连接到境外，再接入动态 IP 池；完善直连、手动填写和选择已有代理的操作与提示。账号名称及已有代理选择的宿主适配已进入本地验证，兼容条件见[适配说明](docs/plugin-host-directory.md)。
- [ ] **在插件内加入一键连接测试。** 手动检查前置代理、动态代理及完整嵌套链路，显示出口、耗时和失败环节，减少复制配置到外部工具排查。
- [ ] **在插件内加入模型路由对照测试。** 同一账号手动对比不加 STATE 与使用 STATE 的响应，展示请求模型、实际返回模型和测试结果。
- [ ] **加入“智力测试” / 能力对照。** 提供固定的推理、代码和 SVG 动画等测试提示词，支持查看、对比生成结果；与模型路由检查分开展示，避免仅凭模型名称就判断能力恢复。
- [ ] **整理测试结果展示与导出。** 在插件页面查看测试记录、对比结果和错误信息，方便复现与反馈，隐藏账号凭据、代理密码和 STATE 原文。

未勾选项是后续计划，不代表当前安装包已经具备；现有测试范围见[插件验证记录](docs/plugin-validation.md)。
...
这里的“防降智”主要针对**响应模型与请求模型不符**的可观测现象；“应对降并发”针对 OpenAI 上游限制账号并发的情况。上游的 Overload、429 和额度限制仍会影响可用性，目前没有并发压测结论；启用账号缺少有效 STATE 时，源码版 / 完整版会让目标模型暂时退出可用调度，插件版则在发往上游前返回暂不可用（503）。
