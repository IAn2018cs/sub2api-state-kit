...
**用于应对最近 ChatGPT / Codex 账号出现的模型降质和降并发问题。** 模型降质主要表现为请求的模型被路由到其他模型；降并发是指 **OpenAI 上游限制账号能同时处理的请求数量**，导致实际可用并发下降。

提供 **增量版本、完整部署版本、插件版本** 三种形式，**优先推荐插件版本**，在支持插件的 Sub2API 中安装即可使用；已有源码修改需求可以选增量版本，新部署可以选完整部署版本，下载入口见下方。

为 Sub2API 增加 **账号级 STATE 管理、Pro / Team 选择和异常动态守护**。可以逐个账号编辑、开启或关闭：只对需要处理的账号启用，正常账号继续按原流程使用。

## 插件 v0.3.4：按分组选择账号，配置、查找、验证都在一个页面

[下载签名插件 v0.3.4](https://github.com/wangyunjeff/sub2api-state-kit/releases/tag/v0.3.4) · [安装指南](docs/plugin.md) · [宿主适配](docs/plugin-host-directory.md)

**下面的完整操作界面需要 Sub2API 0.2.7 + v0.3.4 宿主适配。** 原版 0.2.7 可使用基础配置、自动采集和转发；账号名称、已有代理选择、手动查找及代理 / 模型测试，需要更新宿主适配并重新构建宿主前后端。仅上传新插件不能补齐宿主接口。

**v0.3.4 新增：先选分组，再选账号。** 显示账号名称和 ID，支持全部分组与未分组，已添加账号自动排除；插件停止时也能选择。新增账号默认关闭，保存后再开启。分组只筛选账号，不改变原有分组、代理或调度。

如果操作按钮变灰并提示「插件未运行」，先回到插件管理卡片点击「启用」；「使用 STATE」开关不等于启动插件。
...
先按分组选择并添加账号；账号名称、STATE 开关、查找进度放在同一行。开关点击即保存；单行「查找票据」只启动该账号，也可显式选择全部已开启账号。开启「自动续期与补票」后，按设定提前续期，并在模型不符等异常后重新获取；关闭后只手动操作。

![逐账号开关、单独查找和自动续期入口](docs/images/plugin-v033-accounts.png)

### 2. 先测代理，再查找票据

前置代理可选直连、手动填写或 IP 管理中的已有代理。「测试并应用代理」显示出口 IP、国家和结果，通过后自动保存；首次手动查找优先沿用短时保留的测通会话。单个动态出口失败会按设定次数换出口重试。**出口检测通过、账号授权通过、取得有效 STATE 是三个不同结果，页面分别提示。**

![代理测试、前置代理和清晰的连通性结果](docs/images/plugin-v033-network.png)

**模型请求用哪个代理？** 在 Sub2API「账号管理 → 编辑账号」中设置：选择“无代理”则由服务器直连，选择已有代理则使用该账号代理。插件的动态池只用于采集票据；拿到候选票据后，还要回到账号业务出口复验，日常请求和下方模型测试也用这个业务出口。

只有勾选 **「固定业务代理也经过此前置代理」** 时，列表内账号的固定代理连接才额外经过插件的前置代理（例如 Clash）：`服务器 → Clash → 账号固定代理 → 上游`，最终出口仍由账号固定代理决定；账号选“无代理”时仍直连。该选项独立于 STATE 开关，关闭 STATE 不会关闭这层网络中转。

### 3. 快速检查：重点看请求模型和实际返回模型

选择账号 → 点「快速检查」填入短提示词 → 点「测试模型」。结果用两个醒目的区域并列显示 **请求模型** 与 **实际返回模型**，不必在长日志里找。返回模型不一致会明确标出；同时显示是否实际注入 STATE、HTTP 状态和耗时。勾选「使用当前 STATE」只是请求使用，是否真正注入以结果为准。

<details>
<summary>查看新版界面的模型不匹配提示</summary>

![快速检查：突出请求模型与实际返回模型](docs/images/plugin-v033-quick-check.png)

</details>

### 4. 鹈鹕动画、糖果推理，也能输入自己的提示词

- **鹈鹕动画**：填入 SVG / HTML 动画提示词，生成后可切换 HTML 预览与源码。
- **糖果推理**：填入同一套糖果逻辑题，直接核对 JSON 回答与最坏情况证明。
- 三个预设按钮只填提示词，点击「测试模型」才发送请求；测试中可以停止。

<details>
<summary>查看发布当天的模型不匹配和上游过载案例</summary>

![鹈鹕动画测试与隔离 HTML 预览](docs/images/plugin-v033-pelican.png)

![糖果推理预设与上游错误提示](docs/images/plugin-v033-candy.png)

糖果预设本次实测遇到上游 `server_is_overloaded`，截图如实显示失败原因和「未返回」，不把 HTTP 200 当成完成或模型匹配。上游恢复后可用同一按钮重试。

</details>

以上操作截图来自本地插件，账号和代理信息已脱敏，模型字段保留实际返回值。成功记录与发布当天的异常案例分开标明日期。
...
>
> 日常请求复用当前账号已验证的 STATE，尝试缓解模型被改路由和上游限制并发带来的影响。

| 你关心的问题 | 这里怎么处理 |
| --- | --- |
| **请求大模型，却被路由到 Luna** | 采集和固定代理复验都核对上游返回的模型；通过后才用于业务请求，后续发现模型不符便作废旧 STATE、触发重采 |
| **OpenAI 限制账号并发，能同时处理的请求变少** | 通过采集、复验和复用有效 STATE，尝试改善上游降并发带来的可用性问题；实际恢复程度取决于上游响应 |
| **刚恢复正常，过一会儿又异常** | 到期前自动续期，同时监控 **312 字节 STATE** 和模型不符信号，不只依赖倒计时 |
| **有的账号正常，有的需要处理** | 每个账号独立开关、独立票据，支持手动选择 **Pro / Team** |

这里的“防降智”主要针对**响应模型与请求模型不符**的可观测现象；“应对降并发”针对 OpenAI 上游限制账号并发的情况。上游的 Overload、429 和额度限制仍会影响可用性，目前没有并发压测结论；启用账号缺少有效 STATE 时，源码版 / 完整版会让目标模型暂时退出可用调度，插件版默认无票据也按业务出口放行，只有关闭「无可用票据也放行」时才返回暂不可用（503）。
...
> **最新源码已支持无代理账号：** 全局动态池仍用于采集 STATE；账号选择“无代理”时，复验和日常请求使用当前服务器直连。感谢 [@woai66](https://github.com/woai66) 的修复贡献（[Issue #2](https://github.com/wangyunjeff/sub2api-state-kit/issues/2)）。此更新已进入 `main` 增量源码；已发布的 **v0.2.0 完整部署包不包含此修复**，需用最新源码重新构建。独立插件版已有空代理直连路径，本次无需修改插件包。
...
| **插件版（推荐）** | 已在使用 Sub2API 0.2.7，按需安装手动功能宿主适配 | [插件版 v0.3.4](https://github.com/wangyunjeff/sub2api-state-kit/releases/tag/v0.3.4)，上传 `.s2plugin`，在插件配置页逐账号设置 |

**插件基础功能不需要覆盖宿主源码。** 本文展示的账号名称、已有代理选择及手动查找 / 代理与模型测试，需要[v0.3.4 宿主适配](docs/plugin-host-directory.md)。 首次安装需要向宿主配置追加一次发布者公钥；支持 Linux amd64 / arm64，以及 macOS arm64。全局动态池、可选前置代理、账号开关、Pro / Team、固定出口复验、续期与守护都在插件页操作；支持查看出口 IP 和自动刷新的运行日志。插件与源码版的入口、调度行为和 WebSocket 路径存在区别，详见 **[插件安装指南](docs/plugin.md)** 和 [插件验证记录](docs/plugin-validation.md)。同一个实例选择一种 STATE 实现即可，不建议叠加。
...
本扩展将票据的**本地有效期设为 60 分钟，提前 10 分钟续期**；这是本地管理策略，不是上游承诺的有效期。续期失败时，尚未过期且未被判为异常的旧票据仍可继续使用。
