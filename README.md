...
## 插件 v0.3.3：配置、查找、验证都在一个页面

[下载签名插件 v0.3.3](https://github.com/wangyunjeff/sub2api-state-kit/releases/tag/v0.3.3) · [安装指南](docs/plugin.md) · [宿主适配](docs/plugin-host-directory.md)

**下面的完整操作界面需要 Sub2API 0.2.7 + v0.3.3 宿主适配。** 原版 0.2.7 可使用基础配置、自动采集和转发；账号名称、已有代理选择、手动查找及代理 / 模型测试，需要更新宿主适配并重新构建宿主前后端。仅上传新插件不能补齐宿主接口。
...
账号名称、STATE 开关、查找进度放在同一行。开关点击即保存；单行「查找票据」只启动该账号，也可显式选择全部已开启账号。开启「自动续期与补票」后，按设定提前续期，并在模型不符等异常后重新获取；关闭后只手动操作。
...
| 当前推荐 | **插件 v0.3.3** | 单账号手动操作、自动续期控制、代理测试、模型对照、HTML 预览及糖果推理预设 |
...
| **插件版（推荐）** | 已在使用 Sub2API 0.2.7，按需安装手动功能宿主适配 | [插件版 v0.3.3](https://github.com/wangyunjeff/sub2api-state-kit/releases/tag/v0.3.3)，上传 `.s2plugin`，在插件配置页逐账号设置 |

**插件基础功能不需要覆盖宿主源码。** 本文展示的账号名称、已有代理选择及手动查找 / 代理与模型测试，需要[v0.3.3 宿主适配](docs/plugin-host-directory.md)。 首次安装需要向宿主配置追加一次发布者公钥；支持 Linux amd64 / arm64，以及 macOS arm64。全局动态池、可选前置代理、账号开关、Pro / Team、固定出口复验、续期与守护都在插件页操作；支持查看出口 IP 和自动刷新的运行日志。插件与源码版的入口、调度行为和 WebSocket 路径存在区别，详见 **[插件安装指南](docs/plugin.md)** 和 [插件验证记录](docs/plugin-validation.md)。同一个实例选择一种 STATE 实现即可，不建议叠加。
