...
下面是此前同一套 STATE 流程的真实成功对照：启用 STATE 时，请求模型和上游实际返回模型都为 `gpt-6-astra`；关闭 STATE 的对照行则返回 `gpt-5.6-luna`。这组 Pro / Team 素材与本版失败截图一起保留，分别说明“成功时看什么”和“失败时为什么停”，不把单次成功写成长期保证。

![Pro 成功对照：启用 STATE 后返回模型匹配](docs/images/pro-state-routing.png)

![Team 成功对照：启用 STATE 后返回模型匹配](docs/images/team-state-routing.png)

以上为本地插件的实际操作截图，账号和代理信息已脱敏，模型字段保留实际返回值。截图用于展示操作和结果读取方法；模型名称一致不等于能力评分，也不保证每个账号都能取得有效票据。
