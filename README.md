...
| **插件版（推荐）** | 已在使用官方 Sub2API 0.2.7，希望保留原版宿主 | [插件版 v0.3.1](https://github.com/wangyunjeff/sub2api-state-kit/releases/tag/v0.3.1)，上传 `.s2plugin`，在插件配置页逐账号设置 |

**插件版不需要覆盖宿主源码。** 首次安装需要向宿主配置追加一次发布者公钥；支持 Linux amd64 / arm64，以及 macOS arm64。全局动态池、可选前置代理、账号开关、Pro / Team、固定出口复验、续期与守护都在插件页操作；支持查看出口 IP 和自动刷新的运行日志。插件与源码版的入口、调度行为和 WebSocket 路径存在区别，详见 **[插件安装指南](docs/plugin.md)** 和 [插件验证记录](docs/plugin-validation.md)。同一个实例选择一种 STATE 实现即可，不建议叠加。
